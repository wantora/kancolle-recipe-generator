...
export default class ShareBox extends React.Component {
  render() {
    ...
    const tweetURL =
      ...
      `?hashtags=${encodeURIComponent(hashtags)}&text=${encodeURIComponent(
        text
    ...
    const openTweetURL = (ev) => {
      ...
      window.open(
        tweetURL,
        "_blank",
        `location,resizable,scrollbars,width=${width},height=${height},left=${x},top=${y}`
      );
      ev.preventDefault();
    };
    ...
    return (
      <div className="share-box">
        <a className="tweet-button" href={tweetURL} onClick={openTweetURL}>
          Tweet
        </a>
      </div>
    );
